  ...
  SceneSetting.find()
    ...
    .catch((error) => {
      res
        .status(500)
        .json({
          message: `failed to get scene_settings from server - Error: ${error}`,
        });
    });
});
    ...
    .then((sceneSet) => {
      if (sceneSet) {
        ...
        res
          .status(404)
          .json({
            message: `Could not find an entry with scene_setting_id ${id}`,
          });
      }
    })
    .catch((error) => {
      res
        .status(500)
        .json({
          message: `failed to get scene_setting from server - Error: ${error}`,
        });
    });
});
  ...
  SharedFunc.findById("stories", id)
        ...
        SceneSetting.findByStoryId(id)
          ...
          .catch((error) => {
            res
              .status(500)
              .json({
                message: `Failed to get scene_settings by story id Error: ${error}`,
              });
          });
      } else {
    ...
    .catch((error) => {
      res
        .status(500)
        .json({
          message: `Failed to get scene_settings by project id - Error: ${error}`,
        });
    });
});
  ...
  SharedFunc.add("scene_settings", sceneSettingData)
    ...
    .catch((error) => {
      res
        .status(500)
        .json({
          message: `Failed to create link for scene and setting - Error: ${error}`,
        });
    });
});
        ...
        SharedFunc.update("scene_settings", id, changes)
          ...
          .catch((error) => {
            res
              .status(500)
              .json({
                message: `Error updating the scene_settings connection with server - Error: ${error}`,
              });
          });
      } else {
        res
          .status(404)
          .json({
            message: `Could not find connection between scene and setting`,
          });
      }
    })
    .catch((error) => {
      res
        .status(500)
        .json({ error: `Failed to update scene_Settings connection ${error}` });
    });
});
  ...
  SharedFunc.remove("scene_characters", id)
    .then((deleted) => {
      if (deleted) {
        ...
        res
          .status(404)
          .json({ error: `Could not find scene_setting with id ${id}` });
      }
    })
    .catch((error) => {
      res
        .status(500)
        .json({
          error: `Server unable to delete scene_setting - Error: ${error}`,
        });
    });
});
