...
router.get('/', (req, res) => {
  SharedFunc.findAll('scenes')
    .then(scenes => {
      if(scenes.length == 0){
        res.status(200).json({ message: `There are currently no scenes created`})
      }else{
...
router.post('/', addSceneCheck, (req, res) => {
    ...
    SharedFunc.add('scenes', sceneData)
      ...
      .catch(error => {
          res.status(500).json({ message: `Failed to create a new scene - Error: ${error}` })
      })
})
    ...
    .then(scene => {
      if (scene){
        SharedFunc.update('scenes', id, changes)
          ...
          .catch(error => {
            res.status(500).json({message: `Error updating scene - please confirm the correct info is being sent - Error: ${error}`})
          })
      }else{
        res.status(404).json({ message: `Could not find scene with id ${id}`})
      }
